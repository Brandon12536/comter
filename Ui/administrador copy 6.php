...
<style>
/* Epic table styles with original colors */
.table-responsive {
    padding: 20px;
...
/* Preserve original background colors while adding modern effects */
.table thead th {
    font-weight: 600;
...
.table thead th[style*="background-color:#79f873"] {
    background-color: #79f873 !important;
}

/* Keep original gray background */
.table thead th[style*="background-color:#cfcfcf"] {
    background-color: #cfcfcf !important;
}

/* Keep original light red background */
.table thead th[style*="background-color:#f2d7d5"] {
...
.table tbody tr:hover {
    ...
    background-color: rgba(121, 248, 115, 0.1);
...
.form-select:focus {
    box-shadow: 0 0 0 3px rgba(121, 248, 115, 0.3);
    border-color: #79f873;
}
                ...
                <tr>
                    ...
                    <th>
                        ...
                        <select class="form-select form-select-sm mt-1" id="nombreFilter" onchange="filterTable()">
                            <option value="">Todos</option>
                            <?php
                            $stmt4 = $con->query("SELECT DISTINCT CONCAT(nombre, ' ', apellido) as nombre_completo 
                                                    FROM (
                                                        SELECT nombre, apellido FROM proveedores 
                                                        UNION 
...
function filterTable() {
    ...
    for (let row of rows) {
        ...
        const tipoMatch = !tipo || originalTipo === tipo;
        const companiaMatch = !compania || rowCompania === compania;
        const businessUnitMatch = !businessUnit || rowBusinessUnit === businessUnit;
        const nombreMatch = !nombre || rowNombre === nombre;
        const correoMatch = !correo || rowCorreo === correo;
        const telefonoMatch = !telefono || rowTelefono === telefono;
        const fechaMatch = !fecha || rowFecha === fecha;
        const departamentoMatch = !departamento || rowDepartamento === departamento;
        const puestoMatch = !puesto || rowPuesto === puesto;
        const turnoMatch = !turno || rowTurno === turno;
        
        row.style.display = (tipoMatch && companiaMatch && businessUnitMatch && nombreMatch && 
                           correoMatch && telefonoMatch && fechaMatch && departamentoMatch && 
                           puestoMatch && turnoMatch) ? '' : 'none';
    }
