                    ...
                    <?php if ($stmt->rowCount() > 0): ?>
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)): ?>
                            <tr>
                                ...
                                <td><?php echo !empty($row['departamento']) ? htmlspecialchars($row['departamento']) : 'N/A'; ?></td>
                                <td><?php echo !empty($row['puesto']) ? htmlspecialchars($row['puesto']) : 'N/A'; ?></td>
                                <td><?php echo !empty($row['turno_completo']) ? htmlspecialchars($row['turno_completo']) : 'N/A'; ?></td>
                                <td>
                                    <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editarModal" onclick="cargarDatos(<?php echo $row['id_proveedor']; ?>)">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <a href="javascript:void(0)" class="btn btn-danger btn-sm" onclick="confirmarEliminacion(<?php echo $row['id_proveedor']; ?>)">
    ...
    fetch(`backend/eliminar_registro.php?id_proveedor=${id_proveedor}`)
        ...
        .then(data => {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // Realiza la solicitud de eliminación con el parámetro 'confirmar=1'
                    fetch(`backend/eliminar_registro.php?id_proveedor=${id_proveedor}&confirmar=1`)
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                Swal.fire({
