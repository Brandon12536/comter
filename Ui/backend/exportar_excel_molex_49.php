...
try {
    ...
    // Si es solo validación, retornamos éxito
    if (isset($_GET['validar'])) {
        echo json_encode(["status" => "success", "message" => "Datos disponibles para exportar"]);
        exit;
    }

    // Si llegamos aquí, procedemos con la exportación
    ob_clean();
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment; filename="Molex_SEM49.xlsx"');
    header('Cache-Control: max-age=0');

    // Obtener los datos para el Excel
    $query = "SELECT * FROM molex 
              WHERE id_version = :id_version 
              AND inspection_date BETWEEN :fecha_inicio AND :fecha_fin";
    $stmt = $con->prepare($query);
    $stmt->bindParam(':id_version', $id_version, PDO::PARAM_INT);
    ...
    $headers = [
        ...
        'FUERA DE SPC', 'DAÑO FISICO', 'WIREBOND CORTO', 'WIREBOND CHUECO', 'FISURA',
    ...
    $sheet->getStyle('C' . $totalRowFormula . ':X' . $totalRowFormula)->applyFromArray([
        'borders' => [
            'allBorders' => [
