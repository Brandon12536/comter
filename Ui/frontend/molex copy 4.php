...
<li class="nav-item mt-1">
<button type="button" class="btn btn-primary w-72" id="agregarFila">
                                    <i class="fas fa-plus"></i> Agregar Fila
                                </button>
                                </li>
                                <li class="nav-item mt-1">
                                <button type="button" class="btn btn-success w-73" id="guardarDatos">
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <select class="form-select" id="weekSelect" name="nombre_version" onchange="checkWeekExists(this.value)">
                                <option value="" style="color: #fff;">Seleccione una semana</option>
                                <?php
                                // Obtener la semana de la URL o del formulario enviado
                                $selectedWeek = 'sem34'; // Valor predeterminado
...
<!--<li class="nav-item mt-1">
         ...
         document.addEventListener('DOMContentLoaded', function() {
             ...
             // Establecer el valor del select basado en prioridad:
             // 1. Valor de la URL
             // 2. Valor del POST (ya establecido en el PHP)
             // 3. Valor predeterminado sem34
             if (semanaFromUrl) {
                 weekSelect.value = semanaFromUrl;
                 selectedWeek = semanaFromUrl;
             } else {
                 // Usar el valor ya seleccionado en el select (establecido por PHP)
                 selectedWeek = weekSelect.value;
             }
             
             // Asegurarse de que selectedWeek tenga un valor
             if (!selectedWeek) {
                 selectedWeek = 'sem34';
             ...
             // Agregar listener para los botones de eliminar fila (delegación de eventos)
             document.getElementById('editableTable').addEventListener('click', function(e) {
                 const target = e.target;
                 // Verificar si el clic fue en el botón de eliminar o en su ícono
         ...
         function createEditableRow() {
             ...
             columns.forEach((col, index) => {
                 ...
                 if (col.textContent === 'INSPECTION DATE') {
                     ...
                     // GOODS se calcula automáticamente como la suma de los turnos
                     const input = document.createElement('input');
                     input.type = 'number';
                     ...
                     // Crear select para los campos de turnos
                     ...
                     // Para la columna ACCIONES, solo agregar el botón de eliminar
                     ...
                     // Create select for other numeric fields
                     const select = document.createElement('select');
         ...
         function updateTurnTotals(row) {
             // Calcular la suma de los turnos (1ER T + 2DO T + 3ER T) y actualizar GOODS
             ...
             // Buscar el campo GOODS por su nombre de columna
             ...
             return goodsTotal; // Devolver el total para usarlo en otras funciones
         }
         ...
         function updateTotals(row) {
             ...
             // Asegurarse de que tenemos el valor actualizado de GOODS
             let goodsTotal = updateTurnTotals(row);

             // Buscar los campos TOTAL y TOTAL FINAL por sus nombres de columna
             ...
             if (totalIndex !== -1 && totalFinalIndex !== -1) {
                 ...
                 if (totalInput && totalFinalInput) {
                     // TOTAL muestra la suma de todos los campos de defectos
                     totalInput.value = defectsTotal;
                     
                     // TOTAL FINAL muestra la suma de TOTAL + GOODS
                     totalFinalInput.value = defectsTotal + goodsTotal;
                     
                     // Mostrar los valores calculados en la consola para depuración
         ...
         // Función para actualizar todos los totales de la tabla
         ...
         document.addEventListener('DOMContentLoaded', function() {
             ...
             // Recuperar la semana de la URL o del formulario
             const urlParams = new URLSearchParams(window.location.search);
             const semanaFromUrl = urlParams.get('semana');
             const weekSelect = document.getElementById('weekSelect');
