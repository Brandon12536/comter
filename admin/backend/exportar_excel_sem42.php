...
if ($stmt_version->rowCount() == 0) {
    ...
    $query = "SELECT * FROM inspecciones WHERE id_proveedor = :id_proveedor AND id_version = :id_version";
...
foreach ($records as $record) {
    ...
    $sheet->setCellValue('Q' . $row, $record['coupler_danado'] > 0 ? $record['coupler_danado'] : '');
    $sheet->setCellValue('R' . $row, $record['hundimiento'] > 0 ? $record['hundimiento'] : '');
