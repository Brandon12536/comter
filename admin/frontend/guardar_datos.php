...
try {
    ...
    $stmtAdmin->execute([$id_proveedor]);
    ...
    foreach ($datos as $fila) {
        ...
        try {
            ...
            error_log("Inserción de datos por el administrador ID: {$id_proveedor}");
            

            $hasWirebondValues = ($data['wirebond_corto'] > 0 || $data['wirebond_chueco'] > 0);
            
          
            $data['goods'] = intval($fila['goods'] ?? 0); 
            $data['total'] = intval($fila['total'] ?? 0);  
            $data['total_final'] = intval($fila['total_final'] ?? 0);  
