...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_SESSION['id_proveedor'])) {
        $_SESSION['error_message_sem48'] = 'El proveedor no está disponible. Asegúrate de estar logueado.';
        header('Location: molexsem48.php');
        exit();
    }
    ...
    $fails_coupler_danado = isset($_POST['coupler_danado']) ? (int) $_POST['coupler_danado'] : 0;
    $fails_hundimiento = isset($_POST['hundimiento']) ? (int) $_POST['hundimiento'] : 0;
    ...
    if (!empty($missing_fields)) {
        ...
        $_SESSION['error_message_sem48'] = 'Por favor, rellene los siguientes campos obligatorios: ' . $fields . '.';
        header('Location: molexsem48.php');
        exit();
    }
    ...
    $stmtGetVersion->bindValue(':nombre_version', 'sem48', PDO::PARAM_STR);
    $stmtGetVersion->execute();
    $version = $stmtGetVersion->fetch(PDO::FETCH_ASSOC);

    if (!$version) {
        $sqlInsertVersion = "INSERT INTO versiones_inspeccion (nombre_version) VALUES ('sem48')";
        $stmtInsertVersion = $con->prepare($sqlInsertVersion);
        if ($stmtInsertVersion->execute()) {
            $id_version = $con->lastInsertId();
        } else {
            $_SESSION['error_message_sem47'] = 'Error al insertar la versión sem48 en la base de datos.';
    ...
    $sqlInsert = "INSERT INTO inspecciones 
    ...
    dano_fisico, coupler_danado, hundimiento, fisura, silicon_contaminacion, contaminacion_end_face, 
    ...
    $stmtInsert->bindValue(':coupler_danado', $fails_coupler_danado, PDO::PARAM_INT);
    $stmtInsert->bindValue(':hundimiento', $fails_hundimiento, PDO::PARAM_INT);
    ...
    if ($stmtInsert->execute()) {
        $_SESSION['success_message_sem48'] = 'Record added successfully!';
        header('Location: molexsem48.php');
        exit();
    } else {
        $_SESSION['error_message_sem48'] = 'Error while inserting the record. Please try again.';
        header('Location: molexsem48.php');
        exit();
    }
}
...
$nombre_version = isset($_GET['nombre_version']) ? $_GET['nombre_version'] : 'sem48';
...
function getUniqueValues($column, $con, $id_proveedor)
{
    if ($column === 'nombre_version') {
        $sql = "SELECT DISTINCT vi.nombre_version FROM inspecciones i
                JOIN versiones_inspeccion vi ON i.id_version = vi.id_version
                WHERE vi.nombre_version = 'sem48' AND vi.nombre_version IS NOT NULL 
                AND vi.nombre_version != '' AND i.id_proveedor = :id_proveedor
                ORDER BY vi.nombre_version";
    } else {
        $sql = "SELECT DISTINCT i.$column FROM inspecciones i
                JOIN versiones_inspeccion vi ON i.id_version = vi.id_version
                WHERE vi.nombre_version = 'sem48' AND i.$column IS NOT NULL 
                AND i.$column != '' AND i.id_proveedor = :id_proveedor
                ORDER BY i.$column";
    }
...
$coupler_danado_values = getUniqueValues('coupler_danado', $con, $id_proveedor);
$hundimiento_values = getUniqueValues('hundimiento', $con, $id_proveedor);
    ...
    <a href="semanas.php" class="menu-link">
        ...
        <div data-i18n="Analytics">Nuevo Reporte</div>
                    ...
                    <li class="mt-2">
                        <ul>
                        <div>
                                    <form action="../backend/exportar_excel_sem48.php" method="post">
                                ...
                                <div>
                                    <button type="button" class="btn btn-primary mb-4 add_sem48" data-bs-toggle="modal"
                                        data-bs-target="#addRecordModalsem48" style="width: 100%;">
                                        <i class="bx bx-plus"></i> Agregar registro
                                    </button>
                                </div>
                                <button id="resetFilters" class="btn btn-danger mb-3" style="width: 100%;"><i class="bx bx-reset"></i> Reiniciar Filtros
                                    </button>
                        </ul>
                    </li>
            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    <div class="container my-5">
                        <?php if (isset($_SESSION['success_message_sem48'])): ?>
                            <div class="alert alert-success">
                                <?= $_SESSION['success_message_sem48'];
                                unset($_SESSION['success_message_sem48']); ?>
                            </div>
                        <?php endif; ?>

                        <?php if (isset($_SESSION['error_message_sem48'])): ?>
                            <div class="alert alert-danger">
                                <?= $_SESSION['error_message_sem48'];
                                unset($_SESSION['error_message_sem48']); ?>
                            </div>
                        <?php endif; ?>

                        <div class="container">
                            <h3 class="text-center mt-3">Records MOLEX SEM48</h3>
                            <div class="mt-5 d-flex justify-content-between">
                                ...
                                <div>
                                    <button type="button" class="btn btn-primary mb-4 add_sem48" data-bs-toggle="modal"
                                        data-bs-target="#addRecordModalsem48">
                            ...
                            <div class="table-responsive">
                               <!-- <button id="resetFilters" class="btn btn-danger mb-3"><i class="bx bx-reset"></i> Reset
                                    Filters</button>-->
                                <div style="overflow-y: auto; max-height: 600px;">
                                    ...
                                    <table class="table table-bordered text-center" id="inspectionTable"
                                        ...
                                        <thead
                                            ...
                                            <tr>
                                                ...
                                                <th style="color:#000000; border-color: black;">COUPLER DANADO
                                                    <select id="filter_coupler_danado"
                                                        class="form-select form-select-sm mt-2">
                                                        <option value="">All</option>
                                        ...
                                        <tbody>
                                            ...
                                            if (empty($records)) {
                                                echo '<tr><td colspan="22" style="text-align: center; font-weight: bold;">No hay registros disponibles</td></tr>';
                                            } else {
                                                foreach ($records as $record):
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td style="border-color: black;">
                                                            <?php if ($puedeEditar): ?>
                                                                <a href="editar_inspeccion_sem48.php?id_inspeccion=<?= $record['id_inspeccion']; ?>"
                                                                    class="btn btn-warning edit_sem48">
                                                                    <i class="bx bx-edit"></i> Edit
                                                                </a>
                                                            <?php else: ?>
                                                                <a href="#" class="btn btn-warning edit_sem48 disabled-btn">
                                                                    <i class="bx bx-edit"></i> Edit
                                                                </a>
                                                            <?php endif; ?>
                                                        </td>
                                        ...
                                        <tfoot
                                            style="position: sticky; bottom: 0; background-color: #f2f2f2; z-index: 1;">
                                            <tr>
                                                ...
                                                <td style="border-color: black;">
                                                    <?= number_format($total_coupler_danado, 0) ?>
                                                </td>
                                                <td style="border-color: black;">
                                                    <?= number_format($total_hundimiento, 0) ?>
                                                </td>
                        ...
                        <div class="modal fade" id="addRecordModalsem48" tabindex="-1"
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="molexsem48.php" method="post" enctype="multipart/form-data">
                                            <?php
                      ...
                      multiple>
                                            ...
                                            <div class="mb-3">
                                                <label for="dano_end_face" class="form-label">Daño en el extremo (end
                                                    face)</label>
                                            ...
                                            <div class="mb-3">
                                                <label for="golpe_top" class="form-label">Golpe en la parte
                                                    superior</label>
                                            ...
                                            <div class="mb-3">
                                                <label for="contaminacion_end_face" class="form-label">Contaminación en
                                                    el end face</label>
