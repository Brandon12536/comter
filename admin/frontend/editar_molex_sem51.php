...
if (!isset($_SESSION['id_proveedor'])) {
    $_SESSION['error_message_sem51'] = "La sesión no tiene el id_proveedor.";
    header("Location: molexsem51.php");
    exit();
}

$id_proveedor = $_SESSION['id_proveedor'];
$id_molex = isset($_GET['id_molex']) ? $_GET['id_molex'] : null;

if ($id_molex === null) {
    $_SESSION['error_message_sem51'] = "ID de molex no proporcionado.";
    header("Location: molexsem51.php");
    exit();
}
...
$sql = "SELECT * FROM molex WHERE id_molex = :id_molex AND id_proveedor = :id_proveedor";
$stmt = $con->prepare($sql);
$stmt->bindParam(':id_molex', $id_molex, PDO::PARAM_INT);
$stmt->bindParam(':id_proveedor', $id_proveedor, PDO::PARAM_INT);
$stmt->execute();
...
if ($stmt->rowCount() > 0) {
    ...
    $_SESSION['error_message_sem51'] = "Molex no encontrada.";
    header("Location: molexsem51.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $wirebond_corto = $_POST['wirebond_corto'];
    $wirebond_chueco = $_POST['wirebond_chueco'];
    $fisura = $_POST['fisura'];
    ...
    $sql = "UPDATE molex SET 
            ...
            wirebond_corto = :wirebond_corto,
            wirebond_chueco = :wirebond_chueco,
            fisura = :fisura,
            ...
            comments = :comments
            WHERE id_molex = :id_molex AND id_proveedor = :id_proveedor";
    ...
    $stmt->bindParam(':wirebond_corto', $wirebond_corto, PDO::PARAM_INT);
    $stmt->bindParam(':wirebond_chueco', $wirebond_chueco, PDO::PARAM_INT);
    ...
    if ($stmt->execute()) {
        $_SESSION['success_message_sem51'] = "Molex actualizada correctamente.";
        header("Location: molexsem51.php");
        exit();
    } else {
        $_SESSION['error_message_sem51'] = "Error al actualizar la Molex.";
        header("Location: molexsem51.php");
        exit();
    }
}
...
<head>
    ...
    <title>Editar Molex SEM51</title>
</head>
        ...
        <div data-i18n="Analytics">Nuevo Reporte</div>
            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <h3 class="text-center mt-3">Editar MOLEX SEM51</h3>
                        <form action="editar_molex_sem51.php?id_molex=<?php echo $id_molex; ?>" method="post"
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="operators"
                                    name="operators"><?= htmlspecialchars($inspeccion['operators']) ?></textarea>
                            </div>
                            ...
                            <div class="mb-3">
                                <label for="wirebond_corto" class="form-label">Wirebond corto</label>
                                <select class="form-control" id="wirebond_corto" name="wirebond_corto">
                                    <?php for ($i = 0; $i <= 5000; $i++): ?>
                                        <option value="<?= $i ?>" <?= $i == $inspeccion['wirebond_corto'] ? 'selected' : '' ?>>
                                            <?= $i ?>
                                        </option>
                                    <?php endfor; ?>
                                </select>
                            ...
                            <div class="mb-3">
                                <label for="wirebond_chueco" class="form-label">Wirebond chueco</label>
                                <select class="form-control" id="wirebond_chueco" name="wirebond_chueco">
                                    <?php for ($i = 0; $i <= 5000; $i++): ?>
                                        <option value="<?= $i ?>" <?= $i == $inspeccion['wirebond_chueco'] ? 'selected' : '' ?>><?= $i ?></option>
                                    <?php endfor; ?>
                                </select>
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            fields.forEach(function (field) {
                ...
                if (fieldElement) {
                    fieldElement.addEventListener('input', calculateTotal);
                }
            });
