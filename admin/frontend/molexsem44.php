...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_SESSION['id_proveedor'])) {
        $_SESSION['error_message_sem44'] = 'El proveedor no está disponible. Asegúrate de estar logueado.';
        header('Location: molexsem44.php');
        exit();
    }
    ...
    if (!empty($missing_fields)) {
        ...
        $_SESSION['error_message_sem44'] = 'Por favor, rellene los siguientes campos obligatorios: ' . $fields . '.';
        header('Location: molexsem44.php');
        exit();
    }
    ...
    $stmtGetVersion->bindValue(':nombre_version', 'sem44', PDO::PARAM_STR);
    $stmtGetVersion->execute();
    $version = $stmtGetVersion->fetch(PDO::FETCH_ASSOC);

    if (!$version) {
        $sqlInsertVersion = "INSERT INTO versiones_inspeccion (nombre_version) VALUES ('sem44')";
        $stmtInsertVersion = $con->prepare($sqlInsertVersion);
        if ($stmtInsertVersion->execute()) {
            $id_version = $con->lastInsertId();
        } else {
            $_SESSION['error_message_sem44'] = 'Error al insertar la versión sem44 en la base de datos.';
            header('Location: molexsem44.php');
            exit();
        }
    } else {
    ...
    if ($stmtInsert->execute()) {
        $_SESSION['success_message_sem44'] = 'Record added successfully!';
        header('Location: molexsem44.php');
        exit();
    } else {
        $_SESSION['error_message_sem44'] = 'Error while inserting the record. Please try again.';
        header('Location: molexsem44.php');
        exit();
    }
}
...
$nombre_version = isset($_GET['nombre_version']) ? $_GET['nombre_version'] : 'sem44';
...
function getUniqueValues($column, $con, $id_proveedor)
{
    if ($column === 'nombre_version') {
        $sql = "SELECT DISTINCT vi.nombre_version FROM inspecciones i
                JOIN versiones_inspeccion vi ON i.id_version = vi.id_version
                WHERE vi.nombre_version = 'sem44' AND vi.nombre_version IS NOT NULL 
        ...
        $sql = "SELECT DISTINCT i.$column FROM inspecciones i
                JOIN versiones_inspeccion vi ON i.id_version = vi.id_version
                WHERE vi.nombre_version = 'sem44' AND i.$column IS NOT NULL 
    ...
    <a href="semanas.php" class="menu-link">
        ...
        <div data-i18n="Analytics">Nuevo Reporte</div>
                    ...
                    <li class="mt-2">
                        <ul>
                            <div>
                                <form action="../backend/exportar_excel_sem44.php" method="post">
                            ...
                            <div>
                                <button type="button" class="btn btn-primary mb-4 add_sem44" data-bs-toggle="modal"
                                    data-bs-target="#addRecordModalsem44" style="width: 100%;">
            ...
            height: auto;
                ...
                <div class="content-wrapper">
                    <div class="container my-5">
                        <?php if (isset($_SESSION['success_message_sem44'])): ?>
                            <div class="alert alert-success">
                                <?= $_SESSION['success_message_sem44'];
                                unset($_SESSION['success_message_sem44']); ?>
                            </div>
                        <?php endif; ?>

                        <?php if (isset($_SESSION['error_message_sem44'])): ?>
                            <div class="alert alert-danger">
                                <?= $_SESSION['error_message_sem44'];
                                unset($_SESSION['error_message_sem44']); ?>
                            </div>
                        <?php endif; ?>

                        <div class="container">
                            <h3 class="text-center mt-3">Records MOLEX SEM44</h3>
                            <div class="mt-5 d-flex justify-content-between">
                                ...
                                <div>
                                    <button type="button" class="btn btn-primary mb-4 add_sem44" data-bs-toggle="modal"
                                        data-bs-target="#addRecordModalsem44">
                                            ...
                                            if (empty($records)) {
                                                ...
                                                foreach ($records as $record):
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td style="border-color: black;">
                                                            <?php if ($puedeEditar): ?>
                                                                <a href="editar_inspeccion_sem44.php?id_inspeccion=<?= $record['id_inspeccion']; ?>"
                                                                    class="btn btn-warning edit_sem44">
                                                                    <i class="bx bx-edit"></i> Edit
                                                                </a>
                                                            <?php else: ?>
                                                                <a href="#" class="btn btn-warning edit_sem44 disabled-btn">
                                                                    <i class="bx bx-edit"></i> Edit
                                                                </a>
                                                            <?php endif; ?>
                                                        </td>
                        ...
                        <div class="modal fade" id="addRecordModalsem44" tabindex="-1"
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="molexsem44.php" method="post" enctype="multipart/form-data">
                                            <?php
