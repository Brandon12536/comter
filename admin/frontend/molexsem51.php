...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_SESSION['id_proveedor'])) {
        $_SESSION['error_message_sem51'] = 'El proveedor no está disponible. Asegúrate de estar logueado.';
        header('Location: molexsem51.php');
        exit();
    }
    ...
    if (!empty($missing_fields)) {
        ...
        $_SESSION['error_message_sem51'] = 'Por favor, rellene los siguientes campos obligatorios: ' . $fields . '.';
        header('Location: molexsem51.php');
        exit();
    }
    ...
    $stmtGetVersion->bindValue(':nombre_version', 'sem51', PDO::PARAM_STR);
    $stmtGetVersion->execute();
    $version = $stmtGetVersion->fetch(PDO::FETCH_ASSOC);

    if (!$version) {
        $sqlInsertVersion = "INSERT INTO versiones_inspeccion (nombre_version) VALUES ('sem51')";
        $stmtInsertVersion = $con->prepare($sqlInsertVersion);
        if ($stmtInsertVersion->execute()) {
            $id_version = $con->lastInsertId();
        } else {
            $_SESSION['error_message_sem51'] = 'Error al insertar la versión sem51 en la base de datos.';
            header('Location: molexsem51.php');
            exit();
        }
    } else {
    ...
    if ($stmtInsert->execute()) {
        $_SESSION['success_message_sem51'] = 'Record added successfully!';
        header('Location: molexsem51.php');
        exit();
    } else {
        $_SESSION['error_message_sem51'] = 'Error while inserting the record. Please try again.';
        header('Location: molexsem51.php');
        exit();
    }
}
...
$nombre_version = isset($_GET['nombre_version']) ? $_GET['nombre_version'] : 'sem51';
...
function getUniqueValues($column, $con, $id_proveedor)
{
    if ($column === 'nombre_version') {
        $sql = "SELECT DISTINCT vi.nombre_version FROM molex i
                JOIN versiones_inspeccion vi ON i.id_version = vi.id_version
                WHERE vi.nombre_version = 'sem51' AND vi.nombre_version IS NOT NULL AND vi.nombre_version != ''
                AND i.id_proveedor = :id_proveedor
                ORDER BY vi.nombre_version";
    } else {
        $sql = "SELECT DISTINCT i.$column FROM molex i
                JOIN versiones_inspeccion vi ON i.id_version = vi.id_version
                WHERE vi.nombre_version = 'sem51' AND i.$column IS NOT NULL AND i.$column != ''
                AND i.id_proveedor = :id_proveedor
                ORDER BY i.$column";
    }
        ...
        <div data-i18n="Analytics">Nuevo Reporte</div>
                    ...
                    <li class="mt-2">
                        <ul>
                            ...
                            <div>
                                <form action="../backend/exportar_excel_sem51.php" method="post">
                            ...
                            <div>
                                <button type="button" class="btn btn-primary mb-4 add_sem51" data-bs-toggle="modal"
                                    data-bs-target="#addRecordModalsem51" style="width: 100%;">
            ...
            height: auto;
                ...
                <div class="content-wrapper">
                    <div class="container my-5">
                        <?php if (isset($_SESSION['success_message_sem51'])): ?>
                            <div class="alert alert-success">
                                <?= $_SESSION['success_message_sem51'];
                                unset($_SESSION['success_message_sem51']); ?>
                            </div>
                        <?php endif; ?>

                        <?php if (isset($_SESSION['error_message_sem51'])): ?>
                            <div class="alert alert-danger">
                                <?= $_SESSION['error_message_sem51'];
                                unset($_SESSION['error_message_sem51']); ?>
                            </div>
                        <?php endif; ?>

                        <div class="container">
                            <h3 class="text-center mt-3">Records MOLEX SEM51</h3>
                                    ...
                                    <table class="table table-bordered text-center" id="inspectionTable"
                                        ...
                                        <tbody>
                                            ...
                                            foreach ($records as $record):
                                                ...
                                                <tr>
                                                    ...
                                                    <td style="border-color: black;">
                                                        <?php if ($puedeEditar): ?>
                                                            <a href="editar_molex_sem51.php?id_molex=<?= $record['id_molex']; ?>"
                                                                class="btn btn-warning edit_sem51">
                                                                <i class="bx bx-edit"></i> Edit
                                                            </a>
                                                        <?php else: ?>
                                                            <a href="#" class="btn btn-warning edit_sem51 disabled-btn">
                                                                <i class="bx bx-edit"></i> Edit
                                                            </a>
                                                        <?php endif; ?>
                                                    </td>
                                        ...
                                        <tfoot
                                            ...
                                            <tr>
                                                ...
                                                <td style="border-color: black;">
                                                    <?= number_format($total_dano_end_face, 0) ?></td>
                                                <td style="border-color: black;">
                                                    <?= number_format($total_golpe_top, 0) ?></td>
                                                <td style="border-color: black;"><?= number_format($total_rebaba, 0) ?>
                                                </td>
                                                <td style="border-color: black;">
                                                    <?= number_format($total_dano_en_lente, 0) ?></td>
                                                <td style="border-color: black;">
                                                    <?= number_format($total_fuera_de_spc, 0) ?></td>
                                                <td style="border-color: black;">
                                                    <?= number_format($total_dano_fisico, 0) ?></td>
                                                <td style="border-color: black;">
                                                    <?= number_format($total_wirebond_corto, 0) ?></td>
                                                <td style="border-color: black;">
                                                    <?= number_format($total_wirebond_chueco, 0) ?></td>
                                                <td style="border-color: black;"><?= number_format($total_fisura, 0) ?>
                                                </td>
                                                <td style="border-color: black;">
                                                    <?= number_format($total_silicon_contaminacion, 0) ?></td>
                                                <td style="border-color: black;">
                                                    <?= number_format($total_contaminacion_end_face, 0) ?></td>
                                                <td style="border-color: black;"><?= number_format($total_total, 0) ?>
                                                </td>
                                                <td style="border-color: black;">
                                                    <?= number_format($total_total_final, 0) ?></td>
                            ...
                            <div class="modal fade" id="addRecordModalsem51" tabindex="-1"
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form action="molexsem51.php" method="post" enctype="multipart/form-data">
                                                <?php
