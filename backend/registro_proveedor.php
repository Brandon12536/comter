...
/*if (!preg_match('/@comtermexico\.com$/', $correo)) {
    echo json_encode(['status' => 'error', 'message' => 'Solo se permiten correos con dominio @comtermexico.com']);
    exit();
}*/



if (!preg_match('/(@comtermexico\.com|@gmail\.com)$/', $correo)) {
  echo json_encode(['status' => 'error', 'message' => 'Solo se permiten correos con dominio @comtermexico.com o @pachuca.tecnm.mx']);
  exit();
}

$sql_check_email = "SELECT * FROM proveedores WHERE correo = :correo";
...
try {
    ...
    $sql_insert_temp = "INSERT INTO proveedores (nombre, apellido, compania, business_unit, telefono, correo, codigo_verificacion, password, role, verificado)
